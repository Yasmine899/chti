...
;	la partie reel et imaginaire est en 12.20 
	;la partie rell dans r0 et imaginaire dans r1
	push {lr,r4,r5}
	mov r5,r0
	mov r4,r1
	bl calcul_partie_reel
	mov r1,r4
	mov r4,r0;conserver reel dans r4
	mov r0,r5
	bl calcul_partie_imag
	mov r1,r4	
	asr r0,#16 ;on a le resultat en 12.4
	asr r1,#16 ;;on a le resultat en 12.4
	mul r0,r0,r0;le resultat reel en 24.8
	mul r1,r1,r1;le resultat reel en 24.8
	add r0,r1;on somme pour avoir le res en 24.8
	pop {r4,r5,pc}
	pop {lr}
	endp
...
;first step calcul de la partie réel
calcul_partie_reel 
	push {lr}
	push {r4-r8}
	;on laisse r0 et r1 pour les arguments des fonctions calculant la partie réel et imaginaire
	mov r2,#0 ;notre resultat final en format 12.20
	mov r3,#0 ;l'indice du tableau et boucle i 
	mov r4,#63;pour le "and" logique        
	ldr r5,=TabCos ;format 1.15
boucle_for_reel
;parcourir le tableau
	...
	mul r7,r3,r1 ;r4=k*i   
	;on suppose que c'est ca
	and r7,r4;on fait un "et" logique avec 63 pour le k*i mod 64 ;
	ldrsh r6, [r5,r7, LSL #1];on a la case tabcos[k*i%64] r6 en 1.15
	ldrsh r7,[r0,r3,LSL #1];on a la case signal[i]  r7 en 4.12
	mul r7,r7,r6 ;on multiplie tabcos[k*i%64]*signal[i] ;une retenu =>  r7 en 5.27
	asr r7,#7;r7 en 5.20 valavle pour additionner
	add r2,r2,r7;on rajoute dans r2 le résultat pour cumuler la somme 12.20 (la somme au pire sera en 11.20 +bit de signe donc bien
	add r3,r3,#1;on incremente l'indice de boucle
	cmp r3,#63;pour voir si on sort de la boucle, si inf on réentre
	
	ble boucle_for_reel
	;la boucle est effectue 64 fois qui est 2**6 donc le resultat final sera en 11.27 
	mov r0,r2;on met à jour le signal ech 
	pop {r4-r8}
	pop{lr}
	bx lr
	

;second step calcul de la partie imaginaire
calcul_partie_imag
	push {lr}
	push {r4-r8}
	;on laisse r0 et r1 pour les arguments des fonctions calculant la partie réel et imaginaire
